...
export function signUp(
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      ///---- upto here ---// the code is written so that user automatically get logged in when sign up
...
export function login(email, password, navigate) {
  return async (dispatch) => {
    ...
    try {
      ...
      if(message === "User is not registered, please try again" || message === "Password is incorrect"){
...
export function resetPassword(password, confirmPassword, token, navigate){
  return async(dispatch) =>{
    ...
    try{
      ...
      const message = response.data.message;
      // console.log(message);
      if(message === "Token is invalid" || message === "Token is expired, please regenerate token"){
